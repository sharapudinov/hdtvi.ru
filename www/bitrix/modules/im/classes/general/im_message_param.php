...
class CIMMessageParam
{
	...
	public static function PrepareValues($values, $withDefault = false)
	{
		...
		foreach($values as $key => $value)
		{
			...
			else if (in_array($key, Array('CHAT_ID', 'CHAT_MESSAGE', 'CHAT_LAST_DATE', 'IMOL_VOTE_SID', 'IMOL_VOTE_USER', 'IMOL_VOTE_HEAD', 'SENDING_TS')))
			{
			...
			else if ($key == 'CLASS' || $key == 'IMOL_VOTE' || $key == 'IMOL_VOTE_TEXT' ||  $key == 'IMOL_VOTE_LIKE' ||  $key == 'IMOL_VOTE_DISLIKE')
			{
