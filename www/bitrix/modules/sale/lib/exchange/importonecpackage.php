...
class ImportOneCPackage extends ImportPattern
{
	...
	/**
	 * @param Result $r
	 * @param Exchange\Entity\EntityImport|ProfileImport $item
	 * @return bool
	 * @throws ArgumentException
	 */
	private function importableItems(Result $r, $item)
	{
		...
		elseif($item->getOwnerTypeId() == EntityType::PROFILE)
		{
			/** @var ProfileImport $entity */
			if($r->isSuccess() &&
				!$r->hasWarnings() &&
				!($entity instanceof ProfileImport) &&
				!$item->isImportable())
			{
				$result = false;
			}
