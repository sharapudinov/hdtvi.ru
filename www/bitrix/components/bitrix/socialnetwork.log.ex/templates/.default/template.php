...
if (!$arResult["AJAX_CALL"])
{
	...
	if ($arParams["PUBLIC_MODE"] != "Y")
	{
		...
		if ($arParams["SHOW_REFRESH"] != "N")
		{
			...
			?><div onclick="oLF.refresh('<?=htmlspecialcharsbx(CUtil::JSEscape($refreshUrl))?>')" id="sonet_log_counter_2_container" class="feed-new-message-informer"><?
...
elseif (!$arResult["Events"] || !is_array($arResult["Events"]) || count($arResult["Events"]) <= 0)
{
	ob_end_clean();
...
{
	?><div class="feed-wrap-empty-wrap"><div class="feed-wrap-empty"><?=Loc::getMessage('SONET_C30_T_EMPTY')?></div></div><?
}
